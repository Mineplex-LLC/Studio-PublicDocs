...
You can define environment variables for your game server using the `environmentVariables` property in your `game-properties.yaml` file. These are key-value pairs that are injected into your server's runtime environment.
...
For sensitive values, use the `secretEnvironmentVariableKeys` property. This is an array of keys referencing environment variables that will be converted to secrets when the project is published.

```yaml
secretEnvironmentVariableKeys:
  - DATABASE_PASSWORD
  - API_KEY
```

In this example, the publisher script will read the value of `DATABASE_PASSWORD` and `API_KEY` from the local environment and inject them as secrets into the game server's environment. One way to populate these values is by adding them as GitHub repository secrets on the project, then adding them to your GitHub Actions workflow.

## Compute Settings: Internet Access & OpenTelemetry

The `computeSettings` section allows you to control advanced runtime features:

```yaml
computeSettings:
  enableInternetAccess: true
  enableOpenTelemetryAgent: false
```
...
- `enableOpenTelemetryAgent`: Enables the OpenTelemetry Java agent for distributed tracing and observability. Not compatible with the POLAR anticheat. See [OpenTelemetry Java Agent](https://github.com/open-telemetry/opentelemetry-java-instrumentation) for more info.

## POLAR Anticheat Requirements

If you enable the `POLAR` anticheat in your `anticheats` array, you must meet these requirements:

- `dependencies.libraries` must include `PROTOCOLLIB`
- `computeSettings.enableInternetAccess` must be `true`
- `computeSettings.enableOpenTelemetryAgent` must **NOT** be `true`
